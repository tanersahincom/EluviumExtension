...
export class EditWifiComponent implements OnChanges {
  @ViewChild('wifiForm') wifiForm: NgForm;
  public hide = true;
  ...
  onSubmit () {
    if (this.form.valid) {
      ...
      const encryptedWifiPassword = this.encryptionService.encryptWifi(
        wifiPassword,
        this.password
      )

      this.onePassService
        .AddOrUpdateWifiPassword(request, encryptedWifiPassword)
        .subscribe(p => {
